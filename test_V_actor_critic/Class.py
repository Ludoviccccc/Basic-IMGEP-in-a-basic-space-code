...
class Policy(Frame):
    def __init__(self,N):
        ...
        self.linearTrois = nn.Linear(64, self.N +self.Na) # -1 pour ne pas modifier la dernière ligne
        self.actv = nn.ReLU()
    def forward(self,x:str, logit = False):
        ...
        out = self.actv(out)
        out = self.linearTrois(out)
        dist1 = distributions.Categorical(F.softmax(out[:-self.Na], dim=0))
        line2change = dist1.sample([1]).squeeze()
        dist2 = distributions.Categorical(F.softmax(out[-self.Na:], dim=0))
        ...
        if logit:
            ...
            out = action
        return out
class Epsilon_greedy_policy(Frame):
    ...
    def __call__(self,x:str, logit = False):
        if torch.bernoulli(torch.Tensor([self.epsilon])):
            out = {"line":torch.randint(0,self.N,(1,))[0],"change":torch.randint(0,self.Na,(1,))[0]}
        else:
            out = self.policy(x)
        return out
class ValueFunction(Frame):
    def __init__(self,N):
        ...
        self.linear1 = nn.Linear(self.N,128)
        self.linear2 = nn.Linear(128,64)
        self.linearTrois = nn.Linear(64, 16)
    ...
    def __call__(self,action:dict, program:str):
        ...
        if change==1:
            ...
            if  len(modified_program)<self.N:
                ...
                modified_program[-1] = self.instructions[istr]
                modified_program.append("HLT")
        if change==2 and line2change < len(modified_program)-1:
            #print("modif")
            modified_program[line2change] = 'LHLD'
        if change==3 and line2change < len(modified_program)-1:
            #print("modif")
            modified_program[line2change] = 'SHLD'
        if change==4 and line2change < len(modified_program)-1:
            #print("modif")
            modified_program[line2change] = 'NOP'
...
class Env(CodeModification):
    def __init__(self, N, goal_interference):
        ...
        self.actionRep = torch.eye((self.N)* self.Na).reshape(self.N,self.Na,(self.N)* self.Na)
    ...
    def reward(self,modified_program_signature):
        out = -(self.distance(modified_program_signature)>5)
        return out
    def end(self,program):
        signature = execute(compile_program(program))
        return self.distance(signature)<5
...
class OnlineActorCritic(Env):
    ...
    def UpdatePi(self,advantage, state,action):
        ...
        logit_flat = torch.mul(logits_a[:-self.Na].unsqueeze(1),logits_a[-self.Na:].unsqueeze(0)).flatten()
    ...
    def train(self,start,n_episodes):
        ...
        for j in range(start,n_episodes+1):
                ...
                while not self.end(program) and k<400:
